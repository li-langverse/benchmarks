...
Outputs: `data/latest/benchmark-matrix.json`, `data/latest/benchmark-matrix.md`, `docs/ecosystem/http-server-rps-matrix.md`, `data/latest/summary.json`.

**Comparison table (all scenarios, `li` on every row):** see `docs/ecosystem/http-server-rps-matrix.md` — regenerated by `./scripts/benchmark-matrix-report.py` after tier-5 HTTP benches. Cursor rule: **li-httpd-bench-matrix.mdc**.
