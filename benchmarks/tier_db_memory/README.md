...
# Harness (lidb_embed RSS; requires ../lidb or scripts/lidb-bench-stub fallback):
BENCH_DB_MEMORY_RUN_HARNESS=1 ./scripts/run-db-memory-bench.sh
cat data/latest/tier-db-memory.json
```

Env: `BENCH_DB_MEMORY_PROFILE=ci|nightly`, `BENCH_DB_MEMORY_RUN_HARNESS`, `LIDB_ROOT`, `POSTGRES_URL`, `LIDB_URL`.
