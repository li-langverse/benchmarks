...
**Source:** proactive sweep · discovery `2026-05-29T22:30Z` · CI audit `2026-05-29T22:26Z` · agent-kit audit `2026-05-29T22:29Z` · briefing `2026-05-29T22:07Z`
...
- **Control plane:** heap routes `agent_kit_maintainer` + per-repo `agentkit:*` fingerprints; **no** `org_repo_onboarding` rows — `new_repos` is empty.
...
**Note:** `scripts/discover-new-org-repos.py` was absent on this branch; restored from git `3d49fdf` before refresh. Preflight stdout showing `github=13` was from a run with wrong `ROOT` (script executed outside `benchmarks/`).
