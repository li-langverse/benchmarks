...
export function GpuChipDiagram({
  diagram,
  chipLabel,
  emptyMessage = "No timed workloads on this chip yet — blocked on LKIR emit or lab hardware.",
}: GpuChipDiagramProps) {
          ...
          {series.map((point) => {
            ...
            return (
              <li key={point.workload_id} className="gpu-chip-bar-row">
                ...
                <span className={`badge ${valid === true ? "badge-green" : valid === false ? "badge-red" : "badge-unknown"}`}>
