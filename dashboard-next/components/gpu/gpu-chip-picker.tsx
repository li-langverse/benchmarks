...
/** Stable id for the single visible chip detail tabpanel (all tabs reference this). */
export const GPU_CHIP_PANEL_ID = "gpu-chip-panel";
...
export function GpuChipPicker({
  ...
  const handleTabKeyDown = useCallback(
    (event: KeyboardEvent<HTMLButtonElement>, index: number) => {
      ...
      const next = contributions[nextIndex];
      if (!next) return;
      onSelect(next.chip_slug);
      focusTab(nextIndex);
    },
  ...
  return (
    <section className="gpu-chip-picker" aria-label="Select GPU chip">
      ...
      <div className="gpu-chip-cards">
        <div role="tablist" aria-label="Contributed GPUs" className="gpu-chip-tablist">
          {contributions.map((c, index) => {
            ...
            return (
              <button
                ...
                aria-controls={GPU_CHIP_PANEL_ID}
                tabIndex={active ? 0 : -1}
