...
def catalog_path_findings() -> tuple[list[dict], list[dict]]:
    """Return (actionable gaps, repo-field mismatches)."""
    import tomllib
    ...
    for row in data.get("benchmark", []):
        ...
        if bench_path.is_dir() or bench_path.is_file():
            continue
        bid = row.get("id")
        if repo in ("lic", "lis") and (ROOT / rel).exists():
            repo_mismatch.append(
                {
                    ...
                    "id": bid,
                    "item": (
                        f"catalog id={bid} repo={repo} but path exists under benchmarks: {rel}"
                    ),
                }
            )
            continue
...
def catalog_without_repo_path() -> list[dict]:
    """Legacy: actionable catalog path gaps only."""
    actionable, _ = catalog_path_findings()
    return actionable
...
def main() -> int:
    ...
    catalog_actionable, catalog_repo_mismatch = catalog_path_findings()
    catalog = catalog_actionable
    stubs = package_stubs()
    ...
    report = {
        ...
        "roots": {
            ...
            "lis": str(LIS),
            "lis_present": LIS.is_dir() or (ROOT / "vendor/lis-tier5").is_dir(),
            "benchmarks": str(ROOT),
    ...
    if catalog_repo_mismatch:
        report["recommended_actions"].append(
            {
                "priority": "P2",
                "action": "Fix catalog.toml repo field (path under benchmarks, repo=lic/lis)",
