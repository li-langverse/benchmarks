...
def resolve_tier5_path(bench_id: str) -> str | None:
    """Prefer benchmarks/workloads copy; fall back to vendored lis-tier5 harness."""
    workloads = f"benchmarks/workloads/tier5_http/scenarios/{bench_id}"
    if path_exists_in_repo(workloads):
        return workloads
    vendor = f"vendor/lis-tier5/benchmarks/tier5_http/scenarios/{bench_id}"
    if path_exists_in_repo(vendor):
        return vendor
    return None


def apply_fixes(benchmarks: list[dict], *, verbose: bool) -> tuple[int, int, int, int]:
    repo_fixes = remap_fixes = am_fixes = tier5_fixes = 0
    for b in benchmarks:
        bench_id = b["id"]
        ...
        if bench_id in TIER5_SCENARIO_IDS or (
            rel.startswith("benchmarks/workloads/tier5_http/scenarios/")
        ):
            resolved = resolve_tier5_path(bench_id)
            if resolved and (rel != resolved or repo != "benchmarks"):
                if verbose:
                    print(f"  tier5 {bench_id}: {rel} ({repo}) -> {resolved}")
                b["path"] = resolved
                b["repo"] = "benchmarks"
                tier5_fixes += 1
            continue

        if bench_id == "tier0_stability" and not path_exists_in_repo(rel):
            if b.get("catalog_lifecycle") != "planned":
                if verbose:
                    print(f"  tier0 {bench_id}: mark planned (lic tree absent)")
    ...
    return repo_fixes, remap_fixes, am_fixes, tier5_fixes
...
def main() -> int:
    ...
    repo_fixes, remap_fixes, am_fixes, tier5_fixes = apply_fixes(benchmarks, verbose=True)
    print(
        f"repo={repo_fixes} vertical_remap={remap_fixes} am_path={am_fixes} tier5={tier5_fixes}"
    )

    if args.write and (repo_fixes or remap_fixes or am_fixes or tier5_fixes):
